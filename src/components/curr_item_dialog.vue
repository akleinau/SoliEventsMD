      ...
      <v-card-title class="dialog-title" :class="isMobile ? 'dialog-title__edit' : 'dialog-title'"
        ...
        <div class="col3-container justify-end" style="padding: 0px;">
          ...
          <div class="row3-container">
            ...
            <select v-model="editableItemGroup.Unterkategorie" aria-label="Unterkategorie auswählen">
              <option v-for="option in SUB_CATEGORIES" :value="option.path" :placeholder="editableItemGroup.Unterkategorie">
                {{ option.label }}
              </option>
            </select>
            <v-tooltip aria-hidden="true" :text="dataStore.getSubCategoryName(editableItemGroup.Unterkategorie ?? '')" location="top">
              <template v-slot:activator="{ props }">
                  <img v-if="dataStore.getSubCategorySvg(editableItemGroup.Unterkategorie) != ''" v-bind="props" class="dialog-title__icon" color="var(--color-anthrazit)" :src="dataStore.getSubCategorySvg(editableItemGroup.Unterkategorie)" :alt="dataStore.getSubCategoryAlt(editableItemGroup.Unterkategorie)"/>
                  <v-icon v-else v-bind="props" size="x-large" color="black" class="dialog-title__icon" :aria-label="dataStore.getSubCategoryName(editableItemGroup.Unterkategorie ?? '')">{{ dataStore.getSubCategoryIcon(editableItemGroup.Unterkategorie) }}</v-icon>
              </template>
            </v-tooltip>
          </div>
